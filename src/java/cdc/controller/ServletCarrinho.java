...
//       request.setAttribute("adminEmail", getServletConfig().getInitParameter("adminEmail"));
        ...
        Integer IdPessoa = Integer.parseInt(sessao.getAttribute("usuario_id").toString());
        try {
            dao = new CarrinhoDAO();
...
//            }else 
            if (cmd.equalsIgnoreCase("cadastrar")) {
                ...
                Integer idProdutos = Integer.parseInt(request.getParameter("idProdutos"));

                Carrinho car = new Carrinho();
                car.setPro(idProdutos);
                car.setQtd(1);
                car.setPessoa(IdPessoa);
                if (! new CarrinhoDAO().verificaSeExiste(idProdutos, IdPessoa)) {
                dao.salvar(car);
                    
                }
                rd = request.getRequestDispatcher("/ServletCarrinho?cmd=mostrar");
                ...
                carrinho = carDao.procuraPorPessoa(new Carrinho(null, null, null, IdPessoa));
                
                for(Carrinho carr: carrinho){
                    prod.add(new ProdutoDAO().buscarId(new Produtos(carr.getPro())));
                }
                request.setAttribute("listProduto", prod);
                rd = request.getRequestDispatcher("/Carrinho.jsp");

            } else if (cmd.equalsIgnoreCase("carrinho")) {
                Integer idProd = Integer.parseInt(request.getParameter("idProduto"));
                HttpSession sessao1 = request.getSession(true);
                List<Integer> list;
                Object x = sessao1.getAttribute("carrinho");
                list = (List) x;
                list.add(idProd);
                sessao1.setAttribute("carrinho", list);
//                       Pedidos ped = new Pedidos(idProd);
//                       dao.salvar(ped);
